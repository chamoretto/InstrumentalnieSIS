...
SUM(inner_query.count_of_bought_tickets) as "Заполненное количество
место",
float4div(SUM(inner_query.count_of_bought_tickets),
SUM(inner_query.places_number)) as "Коэф. заполняемости, больше
= лучше"
...
LEFT OUTER JOIN aircraft_type on aircraft.aircraft_type_id =
...
ORDER BY iq.title, array_position(array['winter', 'sprint', 'summer', 'autumn'],
iq.season);

-- BONUS TRACK, запрос из незаявленных. Анализ прибыли рейсов || Рейтинг самых
прибыльных рейсов
SELECT inner_query.voyage_id as "ID Рейса",
inner_query.voyage_title as "Название рейса",
